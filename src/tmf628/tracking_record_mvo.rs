...
pub struct TrackingRecordMvo {
    ...
    pub characteristic: Vec<CharacteristicMvo>,
...
impl std::fmt::Display for TrackingRecordMvo {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        write!(f, "{}", serde_json::to_string(self).unwrap())
    }
}
impl std::ops::Deref for TrackingRecordMvo {
    type Target = Extensible;
    fn deref(&self) -> &Self::Target {
        &self.extensible
    }
}
impl std::ops::DerefMut for TrackingRecordMvo {
