...
#[cfg(test)]
mod test {
    use super::{ProductStockRef, ReservedProductStockRef, ShipmentItem, ShipmentItemActionType, ShipmentRefOrValue, ShipmentTrackingRef};
    ...
    fn test_shipitemaction_deseralize() {
        let shipitemaction : ShipmentItemActionType = serde_json::from_str(SHIPITEMACTION_JSON).unwrap();
    ...
    fn test_resprodstockref_deseralize() {
        let resprodstockref : ReservedProductStockRef = serde_json::from_str(RESPRODSTOCKREF_JSON).unwrap();
