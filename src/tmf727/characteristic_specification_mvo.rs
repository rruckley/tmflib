...
pub struct CharacteristicSpecificationMvo {
    ...
    pub char_spec_relationship: Vec<CharacteristicSpecificationRelationshipMvo>,
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub characteristic_value_specification: Vec<CharacteristicValueSpecificationMvo>,
    ...
    ///Specifies if the value of this characteristic is unique across all entities instantiated from the specification that uses this characteristic. For example, consider a `ProductSpecification` for a set-top box, with a `CharacteristicSpecification` cardID. Each set-top box must have a different value for cardID, so this isUnique attribute would be set to true for the characteristic.
...
impl std::fmt::Display for CharacteristicSpecificationMvo {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        write!(f, "{}", serde_json::to_string(self).unwrap())
    }
}
impl std::ops::Deref for CharacteristicSpecificationMvo {
    type Target = Extensible;
    fn deref(&self) -> &Self::Target {
        &self._extensible
    }
}
impl std::ops::DerefMut for CharacteristicSpecificationMvo {
