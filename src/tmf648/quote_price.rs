...
mod test {
    ...
    fn test_quote_price_inc() {
        ...
        let quote_price = QuotePrice::new("IncPrice").price(price.clone());
        assert_eq!(quote_price.inc_tax(), QUOTE_PRICE);
        assert_eq!(
            quote_price.ex_tax(),
            QUOTE_PRICE / Decimal::from_f32(1.0 + price.tax_rate).unwrap_or_default()
        );
    }
    ...
    fn test_quote_price_period() {
        ...
        assert_eq!(
            quote_price.recurring_charge_period.unwrap().as_str(),
            QUOTE_PERIOD
        );
    }
    ...
    fn test_quote_price_deserialize() {
        ...
        assert_eq!(
            quote_price.description.unwrap().as_str(),
