...
//! 
//! ### URL Generation
//! The URL genreated by this crate follows a specific format:
//! https://<hostname>/<`LIB_PATH`>/<`TMF_VERSION`>/<`TMF_MODULE`>/<`CLASS_PATH`>/<`ID`>
//! 
//! Where:
//! - `LIB_PATH` is defined as a constant in this crate, but can be overridden at build time by setting the `TMF_PATH` environment variable. This is used to form the base path for all HREFs generated by the library.
//! - `TMF_VERSION` is determined by the build feature, either `build-V4` or `build-V5`.
//! - `TMF_MODULE` is determined by the module of the object, e.g. `partyManagement`. It is defined in mod.rs for each module.
//! - `CLASS_PATH` is determined by the class of the object, e.g. `organization`.
//! - `ID` is the unique identifier for the object, usually a UUID.
//! - hostname can get set when using `get_full_href()` function on objects that implement `HasId`.
